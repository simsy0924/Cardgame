  ...
  openCardPicker(G.myHand, `패를 ${actualN}장 버려야 합니다`, actualN, (selected) => {
    ...
    if (villageSelIdx !== undefined && fieldPenguins.length > 0) {
      gameConfirm(
        ...
        (yes) => {
          if (!yes) { doDiscard(); return; }

          openCardPicker(fieldPenguins, '펭귄 마을 ②: 대신 묘지로 보낼 펭귄 몬스터', 1, (sel) => {
            if (sel.length === 0) { doDiscard(); return; }
            const mon = fieldPenguins[sel[0]];

            // 16단계: 펭귄 마을 ②는 replacement EffectDefinition으로 처리한다.
            // 직접 sendToGrave + 레거시 triggerSummonerPenguin2/_tryRecoverPenguinStrikeFromGrave 호출을 중단하고,
            // card-move 이벤트(sentToGrave) → trigger-queue가 후속 유발을 수집하게 한다.
            if (window.HB_PENGUIN_EFFECTS && typeof window.HB_PENGUIN_EFFECTS.handleVillageDiscardReplacement === 'function') {
              const handled = window.HB_PENGUIN_EFFECTS.handleVillageDiscardReplacement({
                gameState: G,
                controller: 'me',
                selectedIndices: indices,
                villageIndex: villageSelIdx,
                targetCard: mon,
                confirmChoice: true,
                after: () => { sendGameState(); renderAll(); checkWinCondition(); },
              });
              if (handled && handled.handled) return;
            }

            // 구버전 fallback. 신엔진이 없을 때만 직접 처리한다.
            if (mon) {
