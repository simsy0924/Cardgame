...
(function initBulgasauiEffectDefinitions(global) {
  ...
  function draw(ctx, n) { let count = 0; for (let i = 0; i < (n || 1); i += 1) { const d = deck(ctx); if (!d.length) break; const top = d[0]; const r = ctx.move.moveCard({ cardId: getCardId(top), controller: my(ctx), from: { controller: my(ctx), zone: ZONES.DECK, index: 0 }, to: { controller: my(ctx), zone: ZONES.HAND }, reason: 'bulgasauiDraw' }); if (!r || r.ok === false) break; count += 1; } if (count) logSafe(`불가사의: ${count}장 드로우`, 'mine'); return count; }
  ...
  function banishFromZone(ctx, controller, zone, card, reason) { const owner = controller || my(ctx); const r = ctx.move.banishCard({ cardId: getCardId(card), controller: owner, from: { controller: owner, zone }, reason: reason || 'bulgasauiBanish' }); return !!(r && r.ok !== false); }
  function sendFieldToGrave(ctx, controller, card, reason) { const owner = controller || my(ctx); const r = ctx.move.sendToGrave({ cardId: getCardId(card), controller: owner, from: { controller: owner, zone: ZONES.FIELD }, reason: reason || 'bulgasauiSendFieldToGrave' }); return !!(r && r.ok !== false); }
  function discardOne(ctx, reason) { const c = hand(ctx)[0]; if (!c) return false; const r = ctx.move.discardCard({ cardId: getCardId(c), controller: my(ctx), from: { controller: my(ctx), zone: ZONES.HAND, index: 0 }, reason: reason || 'bulgasauiDiscard' }); if (r && r.ok !== false) logSafe(`${reason || '불가사의'}: ${getCardId(c)} 버림`, 'mine'); return !!(r && r.ok !== false); }
  function discardSelfAndOne(ctx, cardId, reason) { const h = hand(ctx); const selfIndex = findIndexById(h, cardId); const otherIndex = h.findIndex((_, index) => index !== selfIndex); if (selfIndex < 0 || otherIndex < 0) return false; const self = h[selfIndex]; const other = h[otherIndex]; const r1 = ctx.move.discardCard({ cardId, controller: my(ctx), from: { controller: my(ctx), zone: ZONES.HAND, index: selfIndex }, reason: `${reason || cardId} self` }); if (!r1 || r1.ok === false) return false; const adjustedOtherIndex = otherIndex > selfIndex ? otherIndex - 1 : otherIndex; const r2 = ctx.move.discardCard({ cardId: getCardId(other), controller: my(ctx), from: { controller: my(ctx), zone: ZONES.HAND, index: adjustedOtherIndex }, reason: `${reason || cardId} extra` }); const ok = !!(r2 && r2.ok !== false); if (ok) logSafe(`${reason || cardId}: ${getCardId(self)}와 패 1장을 버림`, 'mine'); return ok; }
  ...
  function banishOpponentOneEach(ctx) { let n = 0; const oh = hand(ctx, opp(ctx)); if (oh.length) { const c = oh[0]; const r = ctx.move.banishCard({ cardId: getCardId(c), controller: opp(ctx), from: { controller: opp(ctx), zone: ZONES.HAND, index: 0 }, reason: 'originBanishOpponentHand' }); if (r && r.ok !== false) n += 1; }
    ...
    const og = grave(ctx, opp(ctx)); if (og.length) { const c = og[0]; const r = ctx.move.banishCard({ cardId: getCardId(c), controller: opp(ctx), from: { controller: opp(ctx), zone: ZONES.GRAVE, index: 0 }, reason: 'originBanishOpponentGrave' }); if (r && r.ok !== false) n += 1; }
    return n; }
  ...
  function banishFieldOrFieldZone(ctx, controller, card) { if (!card) return false; if (card._fieldZone) { const fz = zoneAccess.getFieldZoneCard(stateOf(ctx), controller); if (fz && getCardId(fz) === getCardId(card)) { const r = ctx.move.removeFieldCard({ controller, to: { controller, zone: ZONES.EXILE }, reason: 'bulgasauiBanishFieldZone' }); return !!(r && r.ok !== false); } return false; } return banishFromZone(ctx, controller, ZONES.FIELD, card, 'bulgasauiBanishField'); }
  ...
  add({ id: 'bulgasaui-illuminati-1-send-types-apply', cardId: '불가사의한 일루미나티', effectNo: 1, type: EFFECT_TYPES.QUICK, cardActivationCost: true, zones: [ZONES.HAND], timing: TIMING.EITHER_TURN, tags: [TAGS.COST_SEND_TO_GRAVE, TAGS.GRAVE_SUMMON, TAGS.EXILE_SUMMON, TAGS.DRAW, TAGS.PLACE_FIELD_CARD], text: '덱에서 불가사의 카드를 카드 종류별로 1장까지 묘지로 보내고 보낸 수에 따라 효과를 적용한다.', canResolve: ctx => deck(ctx).some(isBulgasaui), resolve(ctx) { const sentTypes = new Set(); let sent = 0; ['monster', 'magic', 'trap', 'field', 'normal'].forEach(type => { const t = first(deck(ctx), c => isBulgasaui(c) && getCardDef(c).cardType === type && !sentTypes.has(type)); if (t) { if (sendDeckToGrave(ctx, c => getCardId(c) === getCardId(t), '일루미나티①')) { sentTypes.add(type); sent += 1; } } }); if (sent >= 5) { const f = first(grave(ctx).concat(exile(ctx)), isFieldCard); if (f) { const from = grave(ctx).some(c => getCardId(c) === getCardId(f)) ? ZONES.GRAVE : ZONES.EXILE; ctx.move.placeFieldCard({ cardId: getCardId(f), controller: my(ctx), from: { controller: my(ctx), zone: from }, activate: false, reason: 'illuminatiPlaceField' }); } }
      if (sent >= 4) { const t = first(grave(ctx).concat(exile(ctx)), isBulgasaui); if (t) { const from = grave(ctx).some(c => getCardId(c) === getCardId(t)) ? ZONES.GRAVE : ZONES.EXILE; ctx.move.moveCard({ cardId: getCardId(t), controller: my(ctx), from: { controller: my(ctx), zone: from }, to: { controller: my(ctx), zone: ZONES.DECK }, reason: 'illuminatiReturnToDeck' }); } }
