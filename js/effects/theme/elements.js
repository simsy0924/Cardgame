...
(function initElementsEffectDefinitions(global) {
  ...
  function drawOne(ctx, controller, reason) { const owner = normalizeController(controller || my(ctx)); const d = zoneArray(ctx, owner, ZONES.DECK); if (!d.length) return false; const r = ctx.move.moveCard({ cardId: getCardId(d[0]), controller: owner, from: { controller: owner, zone: ZONES.DECK, index: 0 }, to: { controller: owner, zone: ZONES.HAND }, reason: reason || 'elementsDraw' }); return !!(r && r.ok !== false); }
  function discardFirstHand(ctx, controller, reason) { const owner = normalizeController(controller || my(ctx)); const h = zoneArray(ctx, owner, ZONES.HAND); if (!h.length) return false; const r = ctx.move.discardCard({ cardId: getCardId(h[0]), controller: owner, from: { controller: owner, zone: ZONES.HAND, index: 0 }, reason: reason || 'elementsDiscard' }); return !!(r && r.ok !== false); }
  function banishFirstHand(ctx, controller, reason) { const owner = normalizeController(controller || my(ctx)); const h = zoneArray(ctx, owner, ZONES.HAND); if (!h.length) return false; const r = ctx.move.banishCard({ cardId: getCardId(h[0]), controller: owner, from: { controller: owner, zone: ZONES.HAND, index: 0 }, reason: reason || 'elementsBanishHand' }); return !!(r && r.ok !== false); }
  function firstOrSelected(ctx, candidates) { const list = candidates.slice(); if (!list.length) return null; const selected = (ctx && (ctx.selectedCards || ctx.selectedCardIds || ctx.targets)) || []; for (const item of selected) { if (typeof item === 'number' && list[item]) return list[item]; const raw = item && (item.c || item.card || item); const iid = raw && raw._iid; const id = getCardId(raw); const found = list.find(candidate => { const card = candidate && (candidate.c || candidate.card || candidate); if (iid && card && card._iid) return card._iid === iid; return id && getCardId(card) === id; }); if (found) return found; } return list[0]; }
  ...
  function addCounter(ctx, mon, type, amount, sourceEffectId) {
    ...
    if (type === COUNTERS.ELECTRIC) {
      ...
      for (let i = 0; i < times; i += 1) { if (!discardFirstHand(ctx, opp(ctx), 'elementsElectricCounterDiscard')) break; }
    }
    if (type === COUNTERS.WIND) {
      const before = Math.floor(prev / 2); const after = Math.floor(map[type] / 2); const times = Math.max(0, after - before);
      for (let i = 0; i < times; i += 1) { if (!drawOne(ctx, my(ctx), 'elementsWindCounterDraw')) break; }
    }
    if (sourceEffectId) {
