...
(function initLionEffectDefinitions(global) {
  ...
  add({ id: 'lion-roar-send-deck-lion-monsters', cardId: '사자의 포효', effectNo: 1, type: EFFECT_TYPES.ACTIVATION, cardActivationCost: true, zones: [ZONES.HAND], canResolve: ctx => deck(ctx).some(isLionMonster), resolve(ctx) { const targets = deck(ctx).filter(isLionMonster).slice(); const results = targets.map(c => graveSend(ctx, c, ZONES.DECK, ctrl(ctx), 'lionRoarDeckToGrave')); dispatch(ctx); return { ok: results.length > 0, results }; } });
  add({ id: 'lion-hunt-hand-summon-saja', cardId: '사자의 사냥', effectNo: 1, type: EFFECT_TYPES.ACTIVATION, cardActivationCost: true, zones: [ZONES.HAND], canResolve: ctx => hand(ctx).some(isSajaMonster) && hasFieldSpace(ctx), resolve(ctx) { const r = summon(ctx, choose(ctx, hand(ctx), isSajaMonster), ZONES.HAND, 'lionHuntSummon'); dispatch(ctx); return r; } });
  add({ id: 'lion-claw-grave-summon-saja', cardId: '사자의 발톱', effectNo: 1, type: EFFECT_TYPES.ACTIVATION, cardActivationCost: true, zones: [ZONES.HAND], canResolve: ctx => grave(ctx).some(isSajaMonster) && hasFieldSpace(ctx), resolve(ctx) { const r = summon(ctx, choose(ctx, grave(ctx), isSajaMonster), ZONES.GRAVE, 'lionClawSummon'); dispatch(ctx); return r; } });
  add({ id: 'lion-strike-op-hand-return', cardId: '사자의 일격', effectNo: 1, type: EFFECT_TYPES.QUICK, cardActivationCost: true, zones: [ZONES.HAND], timing: TIMING.EITHER_TURN, canResolve: ctx => field(ctx).some(isLionMonster), resolve(ctx) { try { if (typeof sendAction === 'function') sendAction({ type:'forceReturnHand', count: field(ctx).filter(isLionMonster).length, reason:'사자의 일격' }); } catch (_) {} dispatch(ctx); return { ok:true }; } });
  add({ id: 'true-lion-1-send-three-from-deck-and-summon-one', cardId: '진정한 사자', effectNo: 1, type: EFFECT_TYPES.ACTIVATION, cardActivationCost: true, zones: [ZONES.HAND], canResolve: ctx => ['베이비 라이온','젊은 라이온','에이스 라이온'].every(n => deck(ctx).some(c => id(c)===n)) && hasFieldSpace(ctx), resolve(ctx) { const names = ['베이비 라이온','젊은 라이온','에이스 라이온']; const picked = names.map(n => first(deck(ctx), c => id(c)===n)); const summonPick = choose(ctx, picked) || picked[0]; picked.forEach(c => { if (id(c)!==id(summonPick)) addHand(ctx, c, ZONES.DECK, 'trueLionRevealToHand', true); }); const r = summon(ctx, summonPick, ZONES.DECK, 'trueLionSummonSelected'); dispatch(ctx); return r; } });
  ...
  add({ id: 'lonely-lion-1-search', cardId: '고고한 사자', effectNo: 1, type: EFFECT_TYPES.ACTIVATION, cardActivationCost: true, zones: [ZONES.HAND], condition: canUseDeploy, canResolve: ctx => deck(ctx).some(c => isLion(c) || isSaja(c)), resolve(ctx) { const r = addHand(ctx, choose(ctx, deck(ctx), c => isLion(c) || isSaja(c)), ZONES.DECK, 'lonelyLionSearch', true); dispatch(ctx); return r; } });
  add({ id: 'lonely-lion-2-banish-op-field', cardId: '고고한 사자', effectNo: 2, type: EFFECT_TYPES.ACTIVATION, cardActivationCost: true, zones: [ZONES.HAND], condition: canUseDeploy, canResolve: ctx => field(ctx, opp(ctx)).length > 0, resolve(ctx) { const r = banish(ctx, choose(ctx, field(ctx, opp(ctx))), ZONES.FIELD, opp(ctx), 'lonelyLionBanish'); dispatch(ctx); return r; } });
