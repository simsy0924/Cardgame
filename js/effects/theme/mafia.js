...
(function initMafiaEffectDefinitions(global) {
  ...
  function draw(ctx, controller, n) { const ctrl = controller || my(ctx); let count = 0; for (let i = 0; i < (n || 1); i += 1) { const d = deck(ctx, ctrl); if (!d.length) break; const top = d[0]; const r = ctx.move.moveCard({ cardId: getCardId(top), controller: ctrl, from: { controller: ctrl, zone: ZONES.DECK, index: 0 }, to: { controller: ctrl, zone: ZONES.HAND }, reason: 'mafiaDraw' }); if (!r || r.ok === false) break; count += 1; } return { ok: count === (n || 1), count }; }
  ...
  function movePublicToHand(ctx, controller, n, reason) { const ctrl = controller || my(ctx); let count = 0; while (count < (n || 1)) { const cards = publicHand(ctx, ctrl); if (!cards.length) break; const r = ctx.move.moveCard({ cardId: getCardId(cards[0]), controller: ctrl, from: { controller: ctrl, zone: ZONES.PUBLIC_HAND }, to: { controller: ctrl, zone: ZONES.HAND }, reason: reason || 'mafiaPublicToHand' }); if (!r || r.ok === false) break; count += 1; } return count; }
