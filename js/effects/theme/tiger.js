...
(function initTigerEffectDefinitions(global) {
  ...
  add({id:'tiger-roar-banish-deck-tiger-monsters',cardId:'호랑이의 포효',effectNo:1,type:EFFECT_TYPES.ACTIVATION,zones:[ZONES.HAND],canResolve:ctx=>deck(ctx).some(isTigerMonster),resolve(ctx){ const results=deck(ctx).filter(isTigerMonster).slice().map(c=>banish(ctx,c,ZONES.DECK,ctrl(ctx),'tigerRoarBanish')); dispatch(ctx); return {ok:results.length>0,results}; }});
  add({id:'tiger-hunt-hand-summon-horang',cardId:'호랑이의 사냥',effectNo:1,type:EFFECT_TYPES.ACTIVATION,zones:[ZONES.HAND],canResolve:ctx=>hand(ctx).some(isHorangMonster)&&hasFieldSpace(ctx),resolve(ctx){ const r=summon(ctx,choose(ctx,hand(ctx),isHorangMonster),ZONES.HAND,'tigerHuntSummon'); dispatch(ctx); return r; }});
  add({id:'tiger-claw-exile-summon-horang',cardId:'호랑이의 발톱',effectNo:1,type:EFFECT_TYPES.ACTIVATION,zones:[ZONES.HAND],canResolve:ctx=>exile(ctx).some(isHorangMonster)&&hasFieldSpace(ctx),resolve(ctx){ const r=summon(ctx,choose(ctx,exile(ctx),isHorangMonster),ZONES.EXILE,'tigerClawSummon'); dispatch(ctx); return r; }});
  add({id:'tiger-strike-op-field-banish-by-tigers',cardId:'호랑이의 일격',effectNo:1,type:EFFECT_TYPES.QUICK,zones:[ZONES.HAND],timing:TIMING.EITHER_TURN,canResolve:ctx=>field(ctx).some(isTigerMonster)&&field(ctx,opp(ctx)).length>0,resolve(ctx){ const count=field(ctx).filter(isTigerMonster).length; const targets=field(ctx,opp(ctx)).slice(0,count); const results=targets.map(c=>banish(ctx,c,ZONES.FIELD,opp(ctx),'tigerStrikeBanish')); dispatch(ctx); return {ok:results.length>0,results}; }});
  add({id:'true-tiger-1-banish-three-from-deck-summon-one',cardId:'진정한 호랑이',effectNo:1,type:EFFECT_TYPES.ACTIVATION,zones:[ZONES.HAND],canResolve:ctx=>['베이비 타이거','젊은 타이거','에이스 타이거'].every(n=>deck(ctx).some(c=>id(c)===n))&&hasFieldSpace(ctx),resolve(ctx){ const picks=['베이비 타이거','젊은 타이거','에이스 타이거'].map(n=>first(deck(ctx),c=>id(c)===n)); const sm=choose(ctx,picks)||picks[0]; picks.forEach(c=>{ if(id(c)!==id(sm)) banish(ctx,c,ZONES.DECK,ctrl(ctx),'trueTigerBanishCost'); }); const r=summon(ctx,sm,ZONES.DECK,'trueTigerSummon'); dispatch(ctx); return r; }});
  ...
  add({id:'lonely-tiger-1-search',cardId:'고고한 호랑이',effectNo:1,type:EFFECT_TYPES.ACTIVATION,zones:[ZONES.HAND],canResolve:ctx=>deck(ctx).some(c=>isTiger(c)||isHorang(c)),resolve(ctx){ const r=addHand(ctx,choose(ctx,deck(ctx),c=>isTiger(c)||isHorang(c)),ZONES.DECK,'lonelyTigerSearch',true); dispatch(ctx); return r; }});
  add({id:'lonely-tiger-2-banish-op-field',cardId:'고고한 호랑이',effectNo:2,type:EFFECT_TYPES.ACTIVATION,zones:[ZONES.HAND],canResolve:ctx=>field(ctx,opp(ctx)).length>0,resolve(ctx){ const r=banish(ctx,choose(ctx,field(ctx,opp(ctx))),ZONES.FIELD,opp(ctx),'lonelyTigerBanish'); dispatch(ctx); return r; }});
