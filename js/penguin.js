...
function resolvePenguinBubu1() {
  ...
  openCardPicker(targets, `펭귄 부부 ①: 펭귄 카드 최대 ${maxPick}장 서치 (1장 선택 후 확인, 또는 2장 선택)`, maxPick, (sel) => {
    if (sel.length === 0) return;
    // ★ targets 스냅샷 기준으로 선택된 cardId 목록 먼저 추출 (덱 변경 전)
    const selectedIds = sel.map(i => targets[i]?.id).filter(Boolean);
    // 중복 방지: 같은 id가 여러 번 선택됐을 경우 처리
...
function resolvePenguinGlory1(sourceInstanceId) {
  ...
  G.myGrave.push({ id: '펭귄의 영광', name: '펭귄의 영광' });

  // ★ 패에서 '펭귄 용사' 또는 '펭귄의 전설'(패에서 용사 취급) 찾기
  // 펭귄의 전설은 패에서 카드명을 '펭귄 용사'로 취급하므로 영광으로 소환 가능
  // 단, 전설은 "자신 필드에 몬스터가 존재할 경우에만 패에 넣을 수 있다"는 조건이 있음
  // → 이미 패에 있으면 조건 충족된 것
  ...
  if (candidates.length === 0) {
    ...
    openCardPicker(
      ...
      (sel) => {
        if (sel.length > 0) {
          _glorySummonFromHand(candidates[sel[0]].handIdx, candidates[sel[0]].id);
        }
      }
    );
  }
